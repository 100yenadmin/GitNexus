...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  lines.push('');
  lines.push(
    're_index: Run `gitnexus analyze` from the exact installed Electric release if data is stale',
  );
  lines.push('');
...
async function getSetupResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return '# GitNexus\n\nNo repositories indexed. Run: `gitnexus analyze` from the exact installed Electric release in a repository.';
  }
